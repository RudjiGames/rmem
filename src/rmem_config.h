...
/// The compression / file write now runs on a background thread (RMEM_ENABLE_ASYNC_WRITE,
/// see rmem_hook.h), so the CPU cost is largely overlapped with the profiled application.
