...
#elif _MSC_VER && ENABLE_PRELOAD
static uint32_t _memory_thread_heap;
#else
#  ifdef _MSC_VER
...
#elif _MSC_VER && ENABLE_PRELOAD
	return TlsGetValue(_memory_thread_heap);
#else
...
#elif _MSC_VER && ENABLE_PRELOAD
	TlsSetValue(_memory_thread_heap, heap);
#else
